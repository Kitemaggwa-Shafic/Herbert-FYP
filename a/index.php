...
<div id="myModal" class="modal">
        <div class="modal-content">
          ...
          <img src="../images/emblem.jpg" alt="University Emblem" style = "width: 200px; height: 200px; object-fit: cover; border-radius: 50%; margin-top: 50px;">
            ...
            <h4 style="color:#fff"><span>Admin Login Panel</span></h4>
         </div> 
          <hr>
          ...
          <div class="modal-body"> 
            ...
            echo '<form name="createform" method="post" action="">';
              echo ' <input class="tf" type="text" name="a_id" placeholder="Admin ID" required="required">';
              echo ' <input class="tf" type="password" name="password" placeholder="Password">';
              echo ' <input id="createbtn" style="padding: 5px 10px; border: solid 1px; margin: 3px 0; border-radius: 2px; background-color: blue; color: #dddddd; font-size: 150%;" type="submit" name="submit" value="Login">';
            echo '</form>';
            ...
            if (isset($_POST['submit'])) {
                ...
                $check_regno = $conn->query("SELECT * FROM sys_admin WHERE adminID ='$a_ID'");
                $chk_usernm_num = mysqli_num_rows($check_regno);
                    ...
                    $verified = md5($dbSalt . $a_password) == $dbPass; 
                        ...
                        if ($verified === true) {
                          $_SESSION['a_go_uname'] = $a_ID;
                          $_SESSION['a_go_pwd'] = $hashed;
                          $_SESSION['a_go_accountid'] = $l_data['accountID'];
                          header('Location: dashboard.php');
                        } else {
                          echo '<div style = "width: 100%; text-align: center; background-color: white; color: blue; padding: 5px 10px; margin-top: 10px;">Wrong ID or password</div>';
                       }
                    }
            }
           ?>
